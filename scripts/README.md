...
|---|---|---|
| `create-tables-api.py` | Create the 4 core tables (`b2b_region`, `b2b_supplier`, `b2b_canonicalproduct`, `b2b_supplieroffer`) via the Web API `EntityDefinitions` endpoint. Now uses the az-CLI token like every other script (the old MSAL/client-secret/device-code path was removed in audit P6). | MVP1 |
...
| `setup-powerbi.py` | **Optional legacy/demo stub** — provisions the workspace + a detached mock push dataset. Primary analytics path is Dataverse DirectQuery (`powerbi/SETUP.md` Step 2). | MVP2 |
| `requirements-seed.txt` | pip deps for the scripts (`requests` only — MSAL/dotenv dropped in audit P6 once all scripts moved to the az-CLI token). | — |
